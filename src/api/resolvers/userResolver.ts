import {Cat, Credentials, TokenContent, User, UserInput, UserOutput} from '../../types/DBTypes';
...
const userResolver = {
    ...
    Mutation:{
        ...
        updateUser: async (_parent: undefined, args: {user: UserInput}, context: MyContext) => {
            isLoggedIn(context);
            return await fetchData<UserResponse>(`${process.env.AUTH_URL}/users`, {
                method: 'PUT',
                headers: {
        ...
        updateUserAsAdmin: async (_parent: undefined, args: {user: UserInput, id: string}, context: MyContext) => {
            isAdmin(context);
        ...
        deleteUserAsAdmin: async (_parent: undefined, args: {id: string}, context: MyContext) => {
            isAdmin(context);
            return await fetchData<UserResponse>(`${process.env.AUTH_URL}/users/${args.id}`, {
                method: 'DELETE',
                headers: {
